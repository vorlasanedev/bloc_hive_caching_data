...
import 'package:bloc_hive_caching_data/features/home/repositories/home_repository.dart';
...
class HomeBloc extends Bloc<HomeEvent, HomeState> {
  ...
  Future<void> _onHomeCallAppSettingsDataEvent(
    ...
    emit(state.copyWith(newHomeProductsStatus: HomeProductsStatusLoading()));
    ...
    if (dataState is DataSuccess) {
      emit(
        state.copyWith(
          newHomeProductsStatus: HomeProductsStatusCompleted(
    ...
    if (dataState is DataFailed) {
      emit(
        state.copyWith(
          newHomeProductsStatus: HomeProductsStatusError(
            dataState.error as String,
          ),
        ),
      );
    }
